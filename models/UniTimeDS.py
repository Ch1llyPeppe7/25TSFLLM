...
from models.DS import UniTimeDeepSeekR1  # 替换为新的模型类
from transformers import AutoTokenizer  # 替换为 LlamaTokenizer
...
class UniTime(nn.Module):
    def __init__(self, args):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(args.model_path)  # 替换为 LlamaTokenizer
        self.backbone = UniTimeDeepSeekR1.from_pretrained(args.model_path,load_in_8bit=True,device_map='cpu')  # 替换为新的模型类
        self.backbone.model.layers = self.backbone.model.layers[:args.lm_layer_num]  # 替换为 model.h
        ...
        self.d_model = config.hidden_size  # 替换为 hidden_size
        ...
        dec_trans_layer = nn.TransformerEncoderLayer(d_model=self.d_model,
                                                     nhead=config.num_attention_heads,  # 替换为 num_attention_heads
                                                     dim_feedforward=self.d_model * 4, 
                                                     dropout=config.attention_dropout,  # 替换为 attention_probs_dropout_prob
                                                     layer_norm_eps=config.rms_norm_eps,  # 替换为 layer_norm_eps
                                                     batch_first=True,
                                                     norm_first=True)
    ...
    def forward(self, info, x_inp, mask):
        ...
        if len(instruct) > 0:
            ...
            instruct_embed = self.backbone.model.embed_tokens(instruct_ids).repeat(x_token.shape[0], 1, 1)  # 替换为 embed_tokens
            inputs_embeds = torch.cat((instruct_embed, x_token), dim=1)
        else:
            inputs_embeds = x_token

        x_enc = (self.backbone(inputs_embeds=inputs_embeds.half(),output_hidden_states=True)[2][-1]).to(torch.float32)
        #print(x_enc)
